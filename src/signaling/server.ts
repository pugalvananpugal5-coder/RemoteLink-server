...
export function attachSignalingServer(httpServer: HttpServer, config: SignalingServerConfig): void {
  ...
  const tokens = new TokenService(config.tokenSecret);
  const rateLimiter = new RateLimiter(config.rateLimitMaxRequests, config.rateLimitWindowSeconds);
    ...
    socket.on('message', (raw: WebSocket.RawData) => {
      ...
      switch (message.type) {
        ...
        case 'connect_request': {
          if (!registeredDeviceId) return sendUnregisteredError(socket);
        ...
        case 'approve_request': {
          ...
          const session = sessions.createSession(
            ...
            request.toDeviceId,
            config.pendingSessionTtlSeconds,
        ...
        case 'reject_request': {
          if (!registeredDeviceId) return sendUnregisteredError(socket);
          const request = sessions.getPendingRequest(message.requestId);
          if (!request || request.toDeviceId !== registeredDeviceId) return;
          sessions.removePendingRequest(message.requestId);
        ...
        case 'signal_ice_candidate': {
          ...
          const peerDeviceId =
            registeredDeviceId === session.controllerDeviceId ? session.remoteDeviceId : session.controllerDeviceId;
        ...
        case 'end_session': {
          ...
          const session = sessions.getSession(message.sessionId);
          if (!session) return;
          if (!tokens.verify(message.sessionToken, message.sessionId)) return;
